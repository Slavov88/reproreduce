...
class GradientReductionIntegrationTests(unittest.TestCase):
    def test_gradient_discrepancy_survives_composed_reduction(self):
        import torch
        ...
        def source_evaluator(source: str):
            ...
            candidate = (torch.zeros(1),) if discrepancy else (torch.ones(1),)
...
"""
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = reduce(
                script,
                oracle=GradientDifferenceOracle(source_evaluator=source_evaluator),
                timeout=5,
            )
